$(function(){ 
    ...
    if(matchMedia("all and (min-width: 1024px)").matches) {  
    ...
    } else if(matchMedia("(min-width:720px) and (max-width: 1024px)").matches) { 
        /* main gnb fixed */
        $('.header').show(function(){
            $('.main-header').show(function(){ 
                $('.main-contents').css('top','112px'); 
            });
            $(this).hasClass('main-header');  
        });  
        /* 띠배너 beltbanner */ 
        ...
        const str0 = '';
        const str1 = 1;
        $('.guide-list>span').length(str0).css('display','none'); 
        $('.guide-list>span').length(str1).append('<span>></span>'); 
    } 
    /* 모바일 gnb */ 
    $('.header_gnbmenu>li>a').mouseenter(function(){
        return false; 
    ...
    $('.subGnbmenu-tit').click(function(){  
        ...
        $('.gnbmenu_arrow>span').css({'transform':'rotate(0deg) translate(0, -50%)'});
        $('.gnbmenu_arrow').removeClass('on');   
        $('.submenu').stop().slideUp();  

        $('.gnbmenu_arrow>span').eq(Mobilegnb).css({
            'transform':'rotate(-180deg) translate(0, 50%)',  
            'transition':'all .5s'
        });
        $('.gnbmenu_arrow').eq(Mobilegnb).addClass('on');
        $('.submenu').eq(Mobilegnb).stop().slideToggle(); 
